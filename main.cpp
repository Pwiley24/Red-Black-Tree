  ...
  if(position->getLeft() == NULL &&
	...
	if(position->getParent()->getLeft() == position){//position is left child so right is sibling
	  sibSide = 'r';
        ...
        cout << "deleting has no children " << position->getValue() << endl;
    ...
    if(replace != NULL && replace->getColor() == 'b' && replace != root){
      ...
      if(replace->getParent() != NULL){
        ...
        cout << "deleting has no children and recursion" << replace->getValue() << endl;
    ...
    cout << "replace b: " << replace->getValue() << endl;
    ...
    cout << "replace a: " << replace->getValue() << endl;
...
void case_1(Node* replaced, Node* &root){
  ...
  if(replaced->getParent() != NULL &&
    ...
    cout << "replaced sibling" << replaced->getSib(replaced->getParent())->getValue() << endl;
